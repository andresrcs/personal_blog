...
* Configura la memoria para la GPU, en Raspberry Pi OS configúrala en 16 MB, y en Ubuntu configura la división de memoria a "240 240MiB for ARM, 16MiB for VideoCore" (Performance Options/ GPU Memory).
...
Para configurar una IP estática en Raspberry Pi OS, edita este archivo, `sudo nano /etc/dhcpcd.conf`, de acuerdo con tus propias necesidades. Esta es una configuración IP de muestra para una conexión por cable:
...
Para instalar R en tu Pi tienes algunas opciones, cada una con sus pros y sus contras:

-   Los repositorios deb estándar para de tu sistema operativo proporcionan archivos binarios de R que son muy simples de instalar, pero son versiones antiguas de R (4.0.0 para Raspberry Pi OS y 4.1.2 para Ubuntu 22.04.1).

    ```{bash install_r_deb, eval=FALSE}
    sudo apt install r-base
    ```

-   El [Proyecto R4Pi](https://r4pi.org/) proporciona un repositorio deb para archivos binarios precompilados de 32 bits para R desde 2021 y archivos binarios de 64 bits desde septiembre de 2022. Instalar R desde estos archivos binarios precompilados es mucho más rápido que compilarlos tu mismo, y también proporcionan un repositorio con binarios precompilados de paquetes que hace que la instalación de paquetes sea mucho más rápida y sencilla.

    Lamentablemente, sólo son compatibles con Raspberry Pi OS, si has elegido este sistema operativo, te recomiendo usar esta opción ya que las ganancias en velocidad son considerables.
...
-   Siempre puedes compilar R desde la fuente, independientemente del sistema operativo que estés utilizando, esto te ofrece la oportunidad de elegir las opciones de compilación, pero a costa de esperar mucho tiempo para que se complete el proceso de compilación.

    ```{bash install_r_compile, eval=FALSE}
    # Especifíca la version R para instalar
    R_VERSION=4.2.1
...
sed -i '8s/.*/NODE_SHA256=5a6e818c302527a4b1cdf61d3188408c8a3e4a1bbca1e3f836c93ea8469826ce/' ../external/node/install-node.sh # SHAMSUM para node-v16.14.0-linux-arm64.tar.xz
...
sudo nano /lib/systemd/system/shiny-server.service # Pega el siguiente contenido
    #!/usr/bin/env bash
    [Unit]
...
Afortunadamente, en los últimos días, RStudio ha puesto a disposición compilaciones "experimentales" del IDE para arm64 (escribí un [artículo al respecto](/es/2022/08/22/rstudio_ide_arm/)), están compiladas para Ubuntu 18/20 y 22, pero Ubuntu 18/20 también es compatible con Raspberry Pi OS. Puedes consultar las compilaciones diarias disponibles [aquí](https://dailies.rstudio.com/rstudio/elsbeth-geranium/). Elije un enlace de acuerdo con el sistema operativo que estás utilizando, también ten en cuenta que estas compilaciones están destinadas a pruebas, por lo que es probable que tengan errores o sean inestables, si tienes problemas para instalar la última versión disponible, intenta con una versión anterior .

Las siguientes instrucciones usan un enlace para Ubuntu 18/20 (compatible con Raspberry Pi OS), recuerda cambiar el enlace si deseas usar otro sistema operativo o una versión IDE más reciente:
...
-   Configura el reenvío de puertos en tu enrutador/módem:
