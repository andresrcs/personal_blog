...
In order to use AWS services you need to have an account, if you already have one, you can skip this part, if not, to create one go to [https://aws.amazon.com/](https://aws.amazon.com/) and click on "Create an AWS Account", fill in the required information and keep pressing "Continue", you are going to be asked to provide a credit card, but you are not going to be charged for using the products comprised on the "Free Tier". 

Once your account has been created, you are going to be directed to the "AWS Management Console", once there, click on the search button, type "EC2" and select "EC2 Virtual Servers in the Cloud"

![](/post/2021-02-01-aws-server.en_files/aws_ec2.jpg){width=600px height=200px}
...
On the right corner look for "Create Key Pair" and click on it.

![](/post/2021-02-01-aws-server.en_files/aws_create_key.jpg){width=600px height=200px}

You are going to see this dialog box, fill it this way, click on "Create Key Pair" and save the resulting file.

![](/post/2021-02-01-aws-server.en_files/aws_new key.jpg){width=600px height=400px}
...
![](/post/2021-02-01-aws-server.en_files/aws_security_settings.jpg){width=600px height=300px}
...
> :warning: Keep the downloaded files safe and at hand, you are going to need them later. Be careful not to share these files since they could enable other people to generate charges to your credit card.
...
To start configuring the playbooks, you usually first need to define an "inventory" (a list of servers to connect to), but in this case, the `provision_ec2_instance` playbook is going to create one automatically for you when you run it and the other playbooks are going to get an updated inventory from AWS automatically using the `aws_ec2` plugging when needed, this is one of the advantages of working with well supported cloud computing services like AWS, but in order for the `aws_ec2` plugging to do its magic, you need to provide it with your AWS credentials and define your [AWS EC2 region](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions), so edit the `aws_r_server/inventories/aws_ec2.yml` file, and fill your credentials and region on the respective variables.
...
Then you need to configure the path to the ssh key for your AWS account by editing the `aws_r_server/group_vars/aws.yml` file, there are other variables here that you can change if needed but, in most cases the only thing to change is the path to your ssh key.
