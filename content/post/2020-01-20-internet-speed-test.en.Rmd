...
Since you are reading an R related article I'm going to assume you already have your basic R environment set, so we are going to focus on the additional stuff, we need to install the Speedtest's CLI, if you are in a Linux machine, you can do it with this commands on a system terminal: 
...
>:pencil2: Also, in non ARM systems, the first time you run the `speedtest` command you have to accept the license so run it once from the terminal with the `--accept-license` option.
...
>:pencil2: Be aware that the CSV output of `Speedtest CLI` is different when installed on a regular operating system (not ARM distros like Raspbian), the command options and column names are not the same so you would have to modify this script accordingly.

```{r Rscript, eval=FALSE}
#! /usr/bin/env Rscript

# Data adquisition
output <- read.csv(text = system("speedtest --csv-header;speedtest --csv", intern = TRUE))

results <- data.frame(stringsAsFactors = FALSE,
    time = as.POSIXct(Sys.time()),
    ip = as.character(output$IP.Address),
    ping = output$Ping,
    download = output$Download / 1E6, # Convert to Mbps
    upload = output$Upload / 1E6, # Convert to Mbps
    isp = "movistar" # This field is not provided on csv output for ARM systems
)
...
env EDITOR=nano crontab -e
  # Add this line
  0 * * * * /home/pi/speedtest_job.R >> /dev/null 2>&1 # Change the file path as needed
