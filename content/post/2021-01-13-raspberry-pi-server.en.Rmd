...
Now, to set the installation options, you can do it by editing the variables in the `raspberry_pi_server/vars/config_vars.yml` file, although, the default options are fine for most use case scenarios, including reasonable security settings. The options you want to pay attention to here are the password for the PostgreSQL main user and choosing a suitable method for installing R, have in mind that using the [R4Pi Project](https://r4pi.org/) (provides precompiled R binaries) is only possible with Raspberry Pi OS.
...
exposed_ports:
  ...
  - { rule: 'allow', port: 5432, proto: 'tcp' }   # Postgresql (omit if Postgresql is not installed)
  - { rule: 'allow', port: 5432, proto: 'udp' }   # Postgresql (omit if Postgresql is not installed)
...
use_r4pi: true # If set to false R will be compiled from source, if set to true the latest R version will be installed, this is only compatible with Raspberry Pi OS.
r_version: '' # (e.g. '4.2.1') Currently, you can only choose the R version if it's going to be compiled.
