...
Now, if you want to change the default installation settings, you can do it by editing the variables on the `raspberry_pi_server/vars/config_vars.yml` file, although, the default options are fine for most use case scenarios, including reasonable security settings to use on real-world applications.
The only things you most certainly want to change here are the email address for security notifications and the password for the PostgreSQL main user.
...
>:warning: If the Nginx configuration hasn't been set by the `install_basic_services.yml` playbook because you have omitted it, you have to configure the reverse proxy manually or access the services with the specific port (e.g. `http://your_server_ip/3838/your_app_name`)
