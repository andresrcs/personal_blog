...
And my new favorite, you can use a precompiled binary from the [R4Pi](https://r4pi.org/ "R for the Raspberry Pi") project, they are not publicly offering an arm64 binary on their website at the moment (work in progress) but they have been kind enough to make available a link with an experimental arm64 binary for me to try out (big thanks to Mark Sellors [@sellorm](https://twitter.com/sellorm)), it works great with Raspberry Pi OS x64 (Debian 11 based) but sadly it is not compatible with Ubuntu 22.04 (Debian 12 based). This option sacrifices the flexibility of choosing your own compiling options but the gain in installation time (also for R packages) definitely worth it:

```{bash install_r_r4pi, eval=FALSE}
# These are temporary installation instructions until a proper deb repository is set in place
sudo apt install gdebi
wget http://dl.sellorm.com/rpi/r-release_4.2.1-2_arm64.deb
sudo gdebi r-release_4.2.1-2_arm64.deb
sudo chmod 755 /usr/bin/R
```
...
I have tried using an [alternative source](https://github.com/LukeChannings/deno-arm64) for the Deno binary but I can't find an alternative source for the Deno DOM plug-in so I guess we will have to live without Quarto capabilities on arm64 for the time being (or if you are more clever than me you can find out a way around this and share with the rest of us).
