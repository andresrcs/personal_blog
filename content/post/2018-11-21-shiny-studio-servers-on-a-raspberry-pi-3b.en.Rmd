...
git clone --depth 1 --branch v1.5.22.1017 https://github.com/rstudio/shiny-server.git
...
sed -i '8s/.*/NODE_SHA256=2e630e18548627f61eaf573233da7949dc0a1df5eef3f486fa9820c5f6c121aa/' ../external/node/install-node.sh # SHAMSUM for node-v18.18.2-linux-arm64.tar.xz
sed -i 's/linux-x64.tar.xz/linux-arm64.tar.xz/' ../external/node/install-node.sh
sed -i 's/https:\/\/github.com\/jcheng5\/node-centos7\/releases\/download\//https:\/\/nodejs.org\/dist\//' ../external/node/install-node.sh
...
Luckily, in recent days, RStudio has made available "experimental"  builds of the IDE for arm64 (I wrote an [article about it](/2022/08/22/rstudio_ide_arm/)), they are compiled for Ubuntu 20, 22, 24 LTS and RedHat 9 but the Ubuntu 22 and 24 binaries are compatible with Raspberry Pi OS as well. You can check the available daily builds [here](https://dailies.rstudio.com/rstudio/). Choose a link accordingly to the operating system you are using, also, have in mind that these builds are meant for testing so they are likely to have bugs or be unstable, if you have problems installing the latest one available, try with an older version.
...
wget https://s3.amazonaws.com/rstudio-ide-build/server/jammy/arm64/rstudio-server-2024.10.0-daily-236-arm64.deb
sudo gdebi rstudio-server-2024.10.0-daily-236-arm64.deb
sudo rm rstudio-server-2024.10.0-daily-236-arm64.deb
```
