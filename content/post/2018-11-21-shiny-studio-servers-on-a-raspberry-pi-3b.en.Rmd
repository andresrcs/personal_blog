...
Back in 2018, I participated in a forum topic at [Posit Community](https://forum.posit.co/t/raspberry-pi-as-a-viable-shiny-server/18672?u=andresrcs) where [\@jladata](https://twitter.com/jladata) was asking if a Raspberry Pi 3B+ could make it as a viable Shiny server. I actually use a Raspberry Pi for that very purpose so I decided to share with the R community, my experience setting up Shiny and Rstudio Server natively on a Raspberry Pi.
...
sudo nmcli c mod 'Wired connection 1' ipv4.addresses 192.168.1.101/24 ipv4.method manual
...
In the past, the only way to install RStudio Server on a Pi was to compile from source and this was extremely time-consuming (took me 3 days the last time I tried on a Pi 3B+) and very prone to failure because the RStudio IDE was under active development.

Luckily, since then, RStudio has made available "experimental"  builds of the IDE for arm64 (I wrote an [article about it](/2022/08/22/rstudio_ide_arm/)), they are compiled for Ubuntu 22, 24 LTS, RedHat 9 and 10. The Ubuntu 22 and 24 binaries are compatible with Raspberry Pi OS as well. You can check the available daily builds [here](https://dailies.rstudio.com/rstudio/). Choose a link accordingly to the operating system you are using, also, have in mind that these builds are meant for testing so they are likely to have bugs or be unstable, if you have problems installing the latest one available, try with an older version.
...
sudo gdebi rstudio-server-2026.01.0-392-arm64.deb
sudo rm rstudio-server-2026.01.0-392-arm64.deb
```
...
If you are interested, I have an [article](/2021/08/16/port_forwarding/) that provides an example on how to configure port forwarding at home.
